...
const CosmeticStorePopup = ({ isOpen, onClose, cosmetics, totalPoints, onPurchase, onSelect }) => {
    const availableColors = Object.keys(cosmetics.cost || {});
    const elements = ["fridge", "table", "computer", "trash", "board", "clock", "gavel", "background"];
                    ...
                    {availableColors.map((color) => (
                        <div key={color} className="color-block">
                            ...
                            {cosmetics.purchased[color] ? (
                                ...
                                <button onClick={() => onPurchase(color)} disabled={totalPoints < cosmetics.cost[color]}>
                                    Unlock
                                </button>
                            )}
                        </div>
